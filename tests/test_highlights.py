...
def test_history_workbook_values_do_not_color_rows():
    record = variant(history_matches=[{"Tier I": 6, "Germ": 11}], af=0.50)

    assert variant_highlight(record) == ""


def test_germline_highlight_requires_count_above_ten_and_uses_af_strength():
    assert variant_highlight(variant(raw={"Germ": 10}, af=0.8)) == ""
    assert variant_highlight(variant(raw={"Germ": 11}, af=0.3499)) == "germline_low_af"
    assert variant_highlight(variant(raw={"Germ": 11}, af=0.35)) == "germline"
    assert variant_highlight(variant(raw={"Germ": 11}, af=None)) == ""


def test_artifact_highlight_wins_over_priority_highlights():
    record = variant(
        matched_rules=["known_artifact"],
        raw={"Tier I": 6, "Germ": 11},
        af=0.5,
    )
...
def test_missing_af_priority_warning_is_exposed_without_mutating_variant():
    record = variant(raw={"Germ": 11}, af=None)
...
def test_no_other_rows_are_colored():
    ...
    assert variant_highlight(variant(raw={"Tier I": 2, "Tier II": 2, "Germ": 9, "Artf": 0})) == ""
